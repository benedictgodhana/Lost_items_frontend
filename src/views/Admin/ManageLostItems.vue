<template>
  <div>
    ...
    <v-container>
      ...
      >
        <v-card-title style="background: green; color: white" class="text-center"
          ><v-icon>mdi-laptop</v-icon> Lost Items
        </v-card-title>
        <v-data-table
          :headers="headers"
          :items="lostItems"
    ...
    <v-dialog v-model="editDialog" max-width="600">
      <v-card>
        <v-card-title  class="text-center" style="background: green; color: white">Edit Item</v-card-title>
          ...
          <v-list>
            ...
            <v-list-item>
              <v-list-item-content>
                ...
                <v-list-item-subtitle>{{ selectedItem.owner }}</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title>Lost Date</v-list-item-title>
                <v-list-item-subtitle>{{ selectedItem.date_found }}</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
    ...
    <v-dialog v-model="deleteDialog" max-width="600">
      <v-card>
        <v-card-title  class="text-center" style="background: green; color: white">Delete Item</v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { title: 'Lost Date', value: 'date_found' },
        { title: 'Image', value: 'image' }, // Add image header
        { title: 'Status', value: 'status' }, // Add image header
  ...
  methods: {
    ...
    async loadLostItems() {
      ...
      try {
        this.loading = true
        const response = await axios.get(`http://127.0.0.1:8000/api/lostitems/`)
